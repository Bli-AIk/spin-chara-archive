...
SPIN_CHARA_ANDROID_WORK_DIR="${SPIN_CHARA_ANDROID_WORK_DIR:-$SPIN_CHARA_MOD_DIR/.build/android}"
SPIN_CHARA_ANDROID_CACHE_DIR="${SPIN_CHARA_ANDROID_CACHE_DIR:-$SPIN_CHARA_MOD_DIR/.build/cache/love-android-11.5}"

SPIN_CHARA_ANDROID_REPO="${SPIN_CHARA_ANDROID_REPO:-https://github.com/love2d/love-android.git}"
SPIN_CHARA_ANDROID_REF="${SPIN_CHARA_ANDROID_REF:-11.5}"
SPIN_CHARA_ANDROID_APPLICATION_ID="${SPIN_CHARA_ANDROID_APPLICATION_ID:-org.spinchara.template}"
SPIN_CHARA_ANDROID_NAME="${SPIN_CHARA_ANDROID_NAME:-spin-chara}"
SPIN_CHARA_ANDROID_ORIENTATION="${SPIN_CHARA_ANDROID_ORIENTATION:-landscape}"
SPIN_CHARA_ANDROID_VERSION_CODE="${SPIN_CHARA_ANDROID_VERSION_CODE:-1}"
SPIN_CHARA_ANDROID_VERSION_NAME="${SPIN_CHARA_ANDROID_VERSION_NAME:-}"
SPIN_CHARA_ANDROID_ICON="${SPIN_CHARA_ANDROID_ICON:-}"
# Directory mode takes precedence over the single-file SPIN_CHARA_ANDROID_ICON:
# icons named <density>.png (ldpi/mdpi/hdpi/xhdpi/xxhdpi/xxxhdpi); a missing
# density falls back to the nearest available one.
SPIN_CHARA_ANDROID_ICON_DIR="${SPIN_CHARA_ANDROID_ICON_DIR:-$SPIN_CHARA_MOD_DIR/assets/icon/android}"
SPIN_CHARA_ANDROID_NDK_DIR="${SPIN_CHARA_ANDROID_NDK_DIR:-}"
SPIN_CHARA_OUTPUT_BASENAME="${SPIN_CHARA_OUTPUT_BASENAME:-spin-chara}"
SPIN_CHARA_FETCH_SDK="${SPIN_CHARA_FETCH_SDK:-1}"
...
# (SPIN_CHARA_TOOLS_DIR) is only resolved when lib.sh is sourced above.
SPIN_CHARA_ANDROID_SDK_DIR="${SPIN_CHARA_ANDROID_SDK_DIR:-$SPIN_CHARA_TOOLS_DIR/android-sdk}"

read_mod_version() {
    version="$(sed -n 's/^[[:space:]]*"version"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' \
        "$SPIN_CHARA_MOD_DIR/mod.json" | head -n 1)"
...
# SPIN_CHARA_ANDROID_SDK_DIR ($SPIN_CHARA_TOOLS_DIR/android-sdk by
# default — the shared tools dir outside the mod tree, same location the Windows
# launcher build_android.ps1 uses). Disable with SPIN_CHARA_FETCH_SDK=0.
...
install_android_sdk() {
    local sdk="$SPIN_CHARA_ANDROID_SDK_DIR"
    ...
    if [ -f "$sdkmanager_cmd" ] || [ -f "$sdkmanager_cmd.bat" ]; then
        ...
        archive="$SPIN_CHARA_MOD_DIR/.build/cache/commandlinetools-${os}-11076708_latest.zip"
        if [ ! -f "$archive" ]; then
...
ensure_android_sdk() {
    ...
    [ "$SPIN_CHARA_FETCH_SDK" = "1" ] || fail \
        "缺少完整的 Android SDK 且 SPIN_CHARA_FETCH_SDK=0；请设置 ANDROID_SDK_ROOT 指向含 API 34 和 NDK 25.2.9519653 的 SDK"

    # Point sdkmanager at the target dir before installing into it.
    export ANDROID_SDK_ROOT="$SPIN_CHARA_ANDROID_SDK_DIR"
    install_android_sdk
}
...
check_inputs() {
    ...
    # SPIN_CHARA_ANDROID_JAVA_HOME/JAVA_HOME (wrong version is an error) or
    # a matching `java` on PATH, and auto-downloads a portable JDK 17 into
    # $SPIN_CHARA_TOOLS_DIR/jdk17 (shared tools dir) when the box has none.
    ensure_java 17
    ...
    ndk_dir="${SPIN_CHARA_ANDROID_NDK_DIR:-$ANDROID_SDK_ROOT/ndk/25.2.9519653}"
    [ -d "$ndk_dir" ] || fail \
    ...
    SPIN_CHARA_ANDROID_NDK_DIR="$ndk_dir"
    ...
    if [ -n "${SPIN_CHARA_ANDROID_SIGNING_KEYSTORE:-}" ]; then
        ...
        export SPIN_CHARA_ANDROID_SIGNING_KEYSTORE
    fi

    printf '%s' "$SPIN_CHARA_ANDROID_APPLICATION_ID" \
        | grep -Eq '^[A-Za-z][A-Za-z0-9_]*(\.[A-Za-z][A-Za-z0-9_]*)+$' || fail \
        "Invalid Android application id: $SPIN_CHARA_ANDROID_APPLICATION_ID"
    [ -n "$SPIN_CHARA_ANDROID_NAME" ] || fail "Android application name cannot be empty"
    case "$SPIN_CHARA_ANDROID_ORIENTATION" in
        landscape|portrait|sensorLandscape|sensorPortrait) ;;
        *) fail "Unsupported Android orientation: $SPIN_CHARA_ANDROID_ORIENTATION" ;;
    esac
    printf '%s' "$SPIN_CHARA_ANDROID_VERSION_CODE" | grep -Eq '^[1-9][0-9]*$' || fail \
        "Android version code must be a positive integer"

    if [ -z "$SPIN_CHARA_ANDROID_VERSION_NAME" ]; then
        SPIN_CHARA_ANDROID_VERSION_NAME="$(read_mod_version)"
    fi
    [ -n "$SPIN_CHARA_ANDROID_VERSION_NAME" ] || fail "Android version name cannot be empty"
}

ensure_android_source() {
    if [ -d "$SPIN_CHARA_ANDROID_CACHE_DIR/.git" ]; then
        if ! git -C "$SPIN_CHARA_ANDROID_CACHE_DIR" rev-parse --verify --quiet \
            "${SPIN_CHARA_ANDROID_REF}^{commit}" >/dev/null; then
            git -C "$SPIN_CHARA_ANDROID_CACHE_DIR" fetch --depth 1 origin \
                "refs/tags/${SPIN_CHARA_ANDROID_REF}:refs/tags/${SPIN_CHARA_ANDROID_REF}"
        fi
    elif [ -e "$SPIN_CHARA_ANDROID_CACHE_DIR" ]; then
        fail "Android cache path exists but is not a Git checkout: $SPIN_CHARA_ANDROID_CACHE_DIR"
    else
        mkdir -p "$(dirname "$SPIN_CHARA_ANDROID_CACHE_DIR")"
        log "Cloning LÖVE Android ${SPIN_CHARA_ANDROID_REF}"
        git clone --recurse-submodules --depth 1 --branch "$SPIN_CHARA_ANDROID_REF" \
            "$SPIN_CHARA_ANDROID_REPO" "$SPIN_CHARA_ANDROID_CACHE_DIR"
    fi

    git -C "$SPIN_CHARA_ANDROID_CACHE_DIR" checkout --detach "$SPIN_CHARA_ANDROID_REF" >/dev/null
    git -C "$SPIN_CHARA_ANDROID_CACHE_DIR" submodule update --init --recursive
}
...
stage_android_icons() {
    ...
    for density in ldpi mdpi hdpi xhdpi xxhdpi xxxhdpi; do
        [ -f "$SPIN_CHARA_ANDROID_ICON_DIR/$density.png" ] && density_dir_has_icon=1
    done

    if [ -n "$SPIN_CHARA_ANDROID_ICON_DIR" ] && [ "$density_dir_has_icon" = "1" ]; then
        for density in ldpi mdpi hdpi xhdpi xxhdpi xxxhdpi; do
    ...
    if [ -n "$SPIN_CHARA_ANDROID_ICON" ]; then
        [ -f "$SPIN_CHARA_ANDROID_ICON" ] || fail \
            "Android icon does not exist: $SPIN_CHARA_ANDROID_ICON"
        for density in ldpi mdpi hdpi xhdpi xxhdpi xxxhdpi; do
            mkdir -p "$stage/app/src/main/res/drawable-$density"
            cp "$SPIN_CHARA_ANDROID_ICON" \
...
stage_android_source() {
    local stage_dir="$SPIN_CHARA_ANDROID_WORK_DIR/project"

    rm -rf "$stage_dir"
    mkdir -p "$stage_dir"
    # tar instead of rsync (rsync is not available in Git Bash on Windows).
    tar -cf - --exclude='*.git' \
        -C "$SPIN_CHARA_ANDROID_CACHE_DIR" . | tar -xf - -C "$stage_dir"
    mkdir -p "$stage_dir/app/src/embed/assets"
    cp "$SPIN_CHARA_ANDROID_WORK_DIR/love/${SPIN_CHARA_OUTPUT_BASENAME}-release.love" \
    ...
    run_helper patch-android-properties \
        ...
        "$SPIN_CHARA_ANDROID_APPLICATION_ID" \
        "$SPIN_CHARA_ANDROID_NAME" \
        "$SPIN_CHARA_ANDROID_ORIENTATION" \
        "$SPIN_CHARA_ANDROID_VERSION_CODE" \
        "$SPIN_CHARA_ANDROID_VERSION_NAME"
...
build_apk() {
    ...
    apk_output="$SPIN_CHARA_OUTPUT_DIR/${SPIN_CHARA_OUTPUT_BASENAME}-android.apk"
